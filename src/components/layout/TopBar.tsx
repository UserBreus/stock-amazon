...
export function TopBar({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  return (
    <header className="bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl sticky top-0 z-40 border-b border-slate-50 dark:border-slate-900 transition-colors duration-500">
        ...
        <div className="flex items-center gap-6 ml-8">
          ...
          <div className="flex items-center gap-4 pl-6 border-l border-slate-100 dark:border-slate-900">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-black text-blue-950 dark:text-white tracking-tight leading-none mb-1">{user?.user_metadata?.full_name || profile?.email?.split('@')[0] || 'Usuario'}</p>
