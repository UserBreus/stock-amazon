...
async function run() {
    try {
        ...
        const r1 = await fetch('http://localhost:5173/api/sql', {
            method: 'POST',
        ...
        if (zeros.length > 0) {
            ...
            const r2 = await fetch('http://localhost:5173/api/sql', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        console.error("Error communicating with Vite Server:", e.message);
    }
}
run();
