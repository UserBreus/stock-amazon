...
const filePath = path.join(__dirname, 'src/pages/ConfiguracionMaestros.tsx');
let content = fs.readFileSync(filePath, 'utf8');

const old1 = `  const [varProdIds, setVarProdIds] = useState<string[]>([]);
  const [varNombre, setVarNombre] = useState('');
  const [varSku, setVarSku] = useState('');
  const [variantes, setVariantes] = useState<any[]>([]);`;

const new1 = `  const [varProdIds, setVarProdIds] = useState<string[]>([]);
  const [varNombre, setVarNombre] = useState('');
  const [varSku, setVarSku] = useState('');
  const [variantes, setVariantes] = useState<any[]>([]);
...
const old2 = `  const deleteVariante = async (id: any) => {
...
const new2 = `  const deleteVariante = async (id: any) => {
      try {
          toast.loading('Eliminando variante...', { id: 'del-var' });
          await executeAWSQuery(\`DELETE FROM Stock_Variantes WHERE id = '\${id}'\`);
          toast.success('Variante eliminada exitosamente.', { id: 'del-var' });
...
const old3 = `        {activeTab === 'costos_cero' && (
...
const new3 = `        {activeTab === 'costos_cero' && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} className="w-full">
              <GestionCostosCero />
          </motion.div>
        )}
