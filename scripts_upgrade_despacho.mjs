...
const updatedComponent = `import React, { useEffect, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { ArrowUpRight, ArrowRightLeft, Scan, Box, Search, Trash2, Printer, CheckCircle, Zap, AlertCircle, Clock, ZapIcon, Send, ArchiveX } from 'lucide-react';
...
export function DespachoEgresos() {
  ...
  const fetchSolicitudes = async () => {
      try {
         // Fake integration or real if exists. 
         setSolicitudes([]); // For now empty to show layout placeholder
      } catch(e) {}
  };

  const processBarcodeFlash = async (code: string) => {
     if(!code.trim()) return;
     if(!destinoId) return toast.error("Flash Mode: Seleccione destino primero", { duration: 1500 });
     
     setLoadingCode(true);
     try {
         const res = await executeAWSQuery(\`
            SELECT e.*, v.nombre_variante, pm.nombre as producto_nombre
         ...
         if(res && res.length > 0) {
            ...
            if(etq.cantidad_actual <= 0) {
               toast.error("Lote agotado: " + etq.codigo_barras, { duration: 2000 });
            } else if (etq.deposito_id?.toString() === destinoId) {
               toast.error("El lote ya está en el destino especificado", { duration: 2000 });
            } else {
               const remId = 'NULL'; // Quick flash doesn't need formal remito grouping but we could
               const txSql = \`
                   BEGIN TRY
                      BEGIN TRANSACTION;
                      UPDATE Stock_Etiquetas SET deposito_id = \${destinoId} WHERE id = \${etq.id};
                      ...
                      VALUES (\${etq.id}, 'traslado_salida', \${etq.cantidad_actual}, \${etq.deposito_id || 'NULL'}, \${destinoId}, '\${(user as any)?.id || ''}');
                      COMMIT TRANSACTION;
                      SELECT 'OK' as status;
                   END TRY
                   BEGIN CATCH
                   ...
                   END CATCH
               \`;
               await executeAWSQuery(txSql);
               toast.success(\`¡Enviado a \${depositos.find(d=>d.id.toString()===destinoId)?.nombre}!\`, {
                   icon: '⚡',
                   style: { borderRadius: '10px', background: '#333', color: '#fff' }
               });
            }
         } else {
            toast.error("Código Inexistente", { duration: 1500 });
         }
     } catch (e: any) {
         toast.error("Error: " + e.message);
     } finally {
         setLoadingCode(false);
         setScanInput(''); // Always clear to keep scanning!
     }
  };
  ...
  const handleManualCodeSubmit = (e: React.FormEvent) => {
     e.preventDefault();
     if(mode === 'unitario') processBarcodeFlash(scanInput);
     else processBarcodeCart(scanInput);
  };

  const executeBatchOperation = async () => {
      // (Manteniendo la logica robusta del modulo Lotes original)
      if(cart.length === 0) return toast.error("La bandeja está vacía.");
      if(operationType === 'traslado' && !destinoId) return toast.error("Seleccione destino.");
      ...
      try {
          ...
          const opNameIn = isTransfer ? 'traslado_ingreso' : '';
              ...
              if (item.cantidad_a_extraer === item.cantidad_actual) {
                  if (isTransfer) {
                      queries.push(\`
                          ...
                          VALUES (\${item.id}, '\${opNameOut}', \${item.cantidad_a_extraer}, \${item.deposito_id || 'NULL'}, \${destinoId}, @RemId, '\${(user as any)?.id || ''}');
                      \`);
                  } else {
                      queries.push(\`
                          ...
                          VALUES (\${item.id}, '\${opNameOut}', \${item.cantidad_a_extraer}, \${item.deposito_id || 'NULL'}, @RemId, '\${(user as any)?.id || ''}');
                      \`);
                  }
              } else {
                  ...
                  if (isTransfer) {
                      ...
                      queries.push(\`
                          ...
                          INSERT INTO Stock_Movimientos (etiqueta_id, tipo_movimiento, cantidad_afectada, deposito_origen_id, deposito_destino_id, remito_id, usuario_id)
                          VALUES (@NewLote_\${i}, 'fraccionamiento_ingreso', \${item.cantidad_a_extraer}, \${item.deposito_id || 'NULL'}, \${destinoId}, @RemId, '\${(user as any)?.id || ''}');
                          INSERT INTO Stock_Movimientos (etiqueta_id, tipo_movimiento, cantidad_afectada, deposito_origen_id, deposito_destino_id, remito_id, usuario_id)
                          VALUES (\${item.id}, 'fraccionamiento_salida', \${item.cantidad_a_extraer}, \${item.deposito_id || 'NULL'}, \${destinoId}, @RemId, '\${(user as any)?.id || ''}');
                      \`);
                  } else {
                      queries.push(\`
                          INSERT INTO Stock_Movimientos (etiqueta_id, tipo_movimiento, cantidad_afectada, deposito_origen_id, remito_id, usuario_id)
          ...
          setCart([]);
          toast.success("Operación Lote ejecutada.");
      } catch (err: any) {
  ...
  return (
    <div className="space-y-4 font-sans">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
          {[
              { id: 'unitario', icon: ZapIcon, label: 'Modo Flash/Unitario' },
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm overflow-hidden">
          ...
          {/* MODO UNITARIO FLASH */}
          {mode === 'unitario' && (
              <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} className="max-w-2xl mx-auto py-10 text-center">
                  ...
                  <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">Flash Transfer</h2>
                  <p className="text-slate-500 dark:text-slate-400 font-medium mb-10 max-w-md mx-auto">Selecciona tu destino. Cada disparo con la pistola enviará inmediatamente el lote escaneado entero, sin pasos intermedios.</p>
                  
                  <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 text-left mb-8">
                     <label className="text-[10px] uppercase font-black tracking-widest text-slate-500 mb-2 block">1. Seleccionar Cueva/Taller Destino Inicial</label>
                     <select 
                         value={destinoId} onChange={e => setDestinoId(e.target.value)}
                         className="w-full h-14 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 font-bold px-4 outline-none focus:border-indigo-500 transition-colors"
                     >
                         <option value="">-- Elige hacia dónde apuntar --</option>
                  ...
                  <div className="relative">
                      <form onSubmit={handleManualCodeSubmit}>
                          ...
                          <input 
                             type="text" autoFocus disabled={!destinoId || loadingCode} value={scanInput} onChange={e=>setScanInput(e.target.value)}
                             className="w-full h-20 pl-16 pr-6 bg-white dark:bg-slate-900 border-2 border-indigo-200 dark:border-indigo-800 rounded-2xl text-2xl font-mono font-bold outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 transition-all placeholder:text-slate-300 dark:placeholder:text-slate-700"
                             placeholder={destinoId ? "Pistolear Lote Aquí..." : "Elige destino primero"}
                          />
                      </form>
                      {loadingCode && <div className="absolute right-6 top-1/2 -translate-y-1/2 w-8 h-8 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin"/>}
                  </div>
              </motion.div>
          )}
          ...
          {mode === 'lote' && (
              <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="col-span-1 lg:col-span-8 flex flex-col h-[65vh]">
                      <div className="flex gap-4 items-end mb-6 bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                          <form onSubmit={handleManualCodeSubmit} className="flex-1 relative">
                             ...
                             <input type="text" autoFocus disabled={loadingCode} value={scanInput} onChange={e=>setScanInput(e.target.value)}
                                className="w-full h-14 pl-12 bg-white dark:bg-slate-900 border-2 border-indigo-100 dark:border-indigo-900/50 rounded-xl font-mono font-bold"
                                placeholder="Escanear múltiples lotes..." />
                          </form>
                          <button onClick={() => setIsCameraOpen(true)} className="h-14 px-6 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black rounded-xl">Cámara</button>
                      </div>
                  ...
                  <div className="col-span-1 lg:col-span-4 flex flex-col gap-4">
                     <div className="bg-slate-50 dark:bg-slate-950 p-6 border border-slate-200 dark:border-slate-800 rounded-2xl">
                         <label className="text-[10px] font-black uppercase text-slate-500 mb-4 block">Operación Master</label>
                         <div className="grid grid-cols-2 gap-2 mb-6">
                            <button onClick={()=>setOperationType('traslado')} className={cn("py-3 font-bold text-xs rounded-xl border-2 transition-all", operationType === 'traslado' ? "bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-indigo-900/30 dark:border-indigo-500 dark:text-indigo-400": "border-slate-200 dark:border-slate-800 text-slate-400")}>Trasladar</button>
                            <button onClick={()=>setOperationType('venta_consumo')} className={cn("py-3 font-bold text-xs rounded-xl border-2 transition-all", operationType === 'venta_consumo' ? "bg-rose-50 border-rose-500 text-rose-700 dark:bg-rose-900/30 dark:border-rose-500 dark:text-rose-400": "border-slate-200 dark:border-slate-800 text-slate-400")}>Consumir</button>
                         </div>
                         {operationType === 'traslado' && (
                            <select value={destinoId} onChange={e=>setDestinoId(e.target.value)} className="w-full h-12 px-3 border border-indigo-200 bg-white dark:bg-slate-900 dark:border-indigo-800 rounded-xl mb-6 font-bold text-indigo-900 dark:text-indigo-100 outline-none">
                                <option value="">Destino Físico...</option>
                                {depositos.map(d=><option key={d.id} value={d.id}>{d.nombre}</option>)}
                            </select>
                         )}
                         <button onClick={executeBatchOperation} disabled={isExecuting || cart.length === 0} className="w-full bg-indigo-600 text-white font-black py-4 rounded-xl shadow-lg disabled:opacity-50">EJECUTAR ORDEN</button>
                     </div>
                  </div>
              </motion.div>
          )}
      ...
      {isCameraOpen && (
          <div className="fixed inset-0 z-[100] bg-black/95 p-4 flex flex-col backdrop-blur-sm">
             ...
             <div className="flex-1 flex items-center justify-center container mx-auto">
                <BarcodeScanner onScan={(code) => { setIsCameraOpen(false); mode === 'unitario' ? processBarcodeFlash(code) : processBarcodeCart(code); }} />
             </div>
          </div>
      )}
...
fs.writeFileSync('src/components/DespachoEgresos.tsx', updatedComponent);
console.log("Upgraded Despacho to multi-mode capabilities!");
