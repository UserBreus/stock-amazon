...
const configBuilderRegex = /\{\(\(\) => \{\n                const sysButtons = \[[\s\S]*?\}\)\(\)\}/m;
const configBtns = `
...
`.trim();
config = config.replace(configBuilderRegex, configBtns);

fs.writeFileSync('src/pages/ConfiguracionMaestros.tsx', config);
console.log('Restored ConfiguracionMaestros to native UI.');
